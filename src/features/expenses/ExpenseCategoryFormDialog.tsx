...
interface ExpenseCategoryFormDialogProps {
  open: boolean;
  onClose: () => void;
  category: ExpenseCategory | null; // null = creating a new one
  onSubmit: (values: ExpenseCategoryFormValues) => Promise<void>;
}

export function ExpenseCategoryFormDialog({ open, onClose, category, onSubmit }: ExpenseCategoryFormDialogProps) {
  const {
    register,
    handleSubmit,
    reset,
  ...
  const submit = async (values: ExpenseCategoryFormValues) => {
    try {
  ...
  return (
    <Dialog open={open} onClose={handleClose} maxWidth="xs" fullWidth>
      ...
      <Box component="form" id="expense-category-form" onSubmit={handleSubmit(submit)} noValidate>
        <DialogContent>
          <Stack spacing={2.5}>
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          ...
          <Button type="submit" form="expense-category-form" variant="contained" disabled={isSubmitting}>
